...
void InitGame()
{
   ...
   // enemy.hBitmap = (HBITMAP)LoadImageA(NULL, "racket_enemy.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    ...
    Meny.hBitmap = (HBITMAP)LoadImageA(NULL, "Меню.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    loading.hBitmap = (HBITMAP)LoadImageA(NULL, "Загрузка.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    game_death.hBitmap = (HBITMAP)LoadImageA(NULL, "Экрансмерти.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
    inventory2.hBitmap = (HBITMAP)LoadImageA(NULL, "Инвентарь2.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
...
void ShowRacketAndBall()
{
    ...
    const int gap = 8; // расстояние между ракушками
    ...
    if (status == ManeMenu) {
        ...
        ShowBitmap(window.context, Meny.x, Meny.y, Meny.width, Meny.height, Meny.hBitmap); //Меню игры
    
    }
    
    if (status == Loading) {

        ShowBitmap(window.context, loading.x, loading.y, loading.width, loading.height, loading.hBitmap); //Экран загрузки
    ...
    if (status == death_screen) {
        ...
        ShowBitmap(window.context, game_death.x, game_death.y, game_death.width, game_death.height, game_death.hBitmap); //Экран смерти
    ...
    if (status == Inventory) {
        ...
        ShowBitmap(window.context, inventory2.x, inventory2.y, inventory2.width, inventory2.height, inventory2.hBitmap); //Инвентарь
